...
def render_formation_report(report):
    ...
    league_round = regex_first(
        plain,
        r"Campionato lega:\s*G(\d+)",
    )
    opponent = regex_first(
        plain,
        r"Avversario:\s*([^\n]+)",
    )
    ...
    with tab2:
        ...
        with left:
            with st.container(border=True):
                st.subheader("👥 XI consigliato")
                st.text(xi or "Dati non disponibili")
        with right:
            with st.container(border=True):
                st.subheader("🪑 Panchina")
                st.text(bench or "Dati non disponibili")
    ...
    with tab3:
        ...
        with left:
            with st.container(border=True):
                st.subheader("🎛 SAFE / BALANCED / UPSIDE")
                st.text(strategies or "Dati non disponibili")
        with right:
            with st.container(border=True):
                st.subheader("📊 Alternative")
                st.text(alternatives or "Dati non disponibili")
...
def render_trade_report(report):
    ...
    value_after = regex_float(plain, r"Valore:\s*[0-9.]+\s*→\s*([0-9.]+)")
    xi_delta = regex_float(
        plain,
        r"Miglior XI:\s*[0-9.]+\s*→\s*[0-9.]+\s*\(([+\-]?[0-9.]+)\)",
    )
    league_delta = regex_float(plain, r"Campionato:\s*([+\-]?[0-9.]+)\s*pt")
    br_delta = regex_float(plain, r"Battle Royale:\s*([+\-]?[0-9.]+)\s*pt")
    total_delta = regex_float(plain, r"Totale /24:\s*([+\-]?[0-9.]+)\s*pt")
    ...
    with st.container(border=True):
        ...
        st.subheader("🎯 Conclusione")
        st.text(conclusion or "Consulta il report completo.")
...
def render_repair_report(report):
    ...
    refunds = regex_float(plain, r"Rimborsi estero attuali:\s*\+?([0-9.]+) cr")
    available = regex_float(plain, r"Disponibile stimato:\s*([0-9.]+) cr")
    price_scale = regex_float(plain, r"Scala prezzi estate\s*→\s*febbraio:\s*([0-9.]+)")
    ...
    for role in ("P", "D", "C", "A"):
        ...
        priorities[role] = value

    st.subheader("🎯 Priorità reparti")
    cols = st.columns(4)
    role_names = {"P": "Portieri", "D": "Difesa", "C": "Centrocampo", "A": "Attacco"}

    for col, role in zip(cols, ("P", "D", "C", "A")):
        value = priorities[role]
        with col:
            st.write(f"**{role_names[role]}**")
            if value is not None:
                st.progress(min(max(value / 100.0, 0), 1), text=f"{value:.0f}/100")
            else:
                st.caption("n/d")
    ...
    with tab1:
        ...
        with st.container(border=True):
            st.text(cuts or "Nessun dato disponibile")
    ...
    with tab2:
        ...
        with st.container(border=True):
            st.text(plan or "Nessun piano disponibile")

    with tab3:
        aggressive = section_text(plain, "PIANO A", ("PIANO B",))
        balanced = section_text(plain, "PIANO B", ("PIANO C",))
        value = section_text(plain, "PIANO C", ("PIANO CONSIGLIATO OGGI",))

        t1, t2, t3 = st.tabs(["🔥 Aggressivo", "⚖️ Bilanciato", "💎 Value"])
        with t1:
            st.text(aggressive or "Nessun dato")
        with t2:
            st.text(balanced or "Nessun dato")
        with t3:
            st.text(value or "Nessun dato")
...
def parse_simulation_targets(plain):
    ...
    for raw in lines:
        ...
        if line.startswith("⚔️"):
            current["rivals"] = line.replace("⚔️ Rivali principali:", "").strip()
...
def render_simulation_report(report):
    ...
    simulations = regex_first(plain, r"Simulazioni eseguite:\s*([0-9]+)")
    teams = regex_first(plain, r"Squadre simulate:\s*([0-9]+)")
    budget = regex_first(plain, r"Budget base per squadra:\s*([^\n]+)")
        ...
        for target in targets[:8]:
            with st.container(border=True):
                ...
                with left:
                    ...
                    st.write(
                        f"TV **{target.get('tv', 0):.1f}** · Scout **{target.get('scout', 0):.1f}** · Breakout **{target.get('breakout', 0):.1f}**"
                    )
                ...
                with right:
                    ...
                    st.metric("Prob. acquisizione", f"{probability:.0f}%")
                    st.progress(
                        min(max(probability / 100.0, 0), 1),
...
def render_control_center_report(report):
    ...
    league_pts = regex_float(competition, r"Punti attesi:\s*([0-9.]+)/3")
    battle_pts = regex_float(competition, r"Battle Royale:\s*([0-9.]+)/21")
    total = regex_float(competition, r"Totale:\s*([0-9.]+)/24")
    ...
    budget = regex_float(auction, r"Budget oggi stimato:\s*([0-9.]+) cr")
    ...
    c3.metric("Campionato", f"{league_pts:.2f}/3" if league_pts is not None else "n/d")
    c4.metric("Battle", f"{battle_pts:.2f}/21" if battle_pts is not None else "n/d")
    c5.metric("Budget Febbraio", f"{budget:g} cr" if budget is not None else "n/d")

    if total is not None:
        st.progress(
            min(max(total / 24.0, 0), 1),
    ...
    sections = (
        section_text(plain, "FORMAZIONE CONSIGLIATA", ("COMPETIZIONI",)),
    ...
    for tab, text in zip(tabs[:5], sections):
        with tab:
            with st.container(border=True):
                st.text(text or "Nessun dato disponibile")
...
with st.sidebar:
    ...
    st.caption("Dashboard V3 · interfaccia grafica")
...
if page == "Home":
    ...
    c4.metric("Versione", "Dashboard V3")
    ...
    for start in range(0, len(cards), 2):
        cols = st.columns(2)
        for col, (title, destination, description) in zip(cols, cards[start:start + 2]):
            with col:
                with st.container(border=True):
                    st.markdown(f"### {title}")
    ...
    st.title("🏆 Formazione")
    st.write("Consiglio aggiornato con una vista sintetica e grafica.")
    ...
    st.write(
        "Scrivi i giocatori separandoli con `+`. Il motore individua automaticamente la controparte."
    )
    ...
    if st.button(
        ...
        if not give.strip() or not receive.strip():
            ...
            try:
                ...
                st.session_state["trade_report_v3"] = report
                st.session_state["trade_report_v3_time"] = datetime.now().strftime(
                    "%d/%m/%Y %H:%M"
                )
            except Exception as exc:
                render_exception(exc)

    if st.session_state.get("trade_report_v3"):
        st.caption(
            "Ultima analisi locale: "
            + st.session_state.get("trade_report_v3_time", "")
        )
        render_trade_report(st.session_state["trade_report_v3"])


elif page == "Talent Scout":
    st.title("🕵️ Talent Scout")
    st.write("Vista locale dello stato Scout aggiornato automaticamente da GitHub Actions.")
    ...
    if not active:
        ...
        for index, player in enumerate(active[:6]):
            with columns[index % 3]:
                with st.container(border=True):
                    st.markdown(f"### {index + 1}. {player.get('name', 'N/D')}")
                    st.caption(f"{player.get('role', '?')} · {player.get('club', 'N/D')}")
                    st.metric(
                        "Scout Score",
                        f"{safe_float(player.get('scout_score')):.1f}",
                    )
                    st.write(
                        f"Breakout **{safe_float(player.get('breakout_score')):.1f}** · Tit. **{safe_float(player.get('probability')):.0f}%**"
                    )
                    st.write(
                        f"7g **{trend_label(player.get('trend_7'))}** · 30g **{trend_label(player.get('trend_30'))}**"
                    )
                    st.caption(
    ...
    st.write("Simula la concorrenza delle altre sette squadre sui target di febbraio.")
    st.caption(
    ...
    st.write("Riepilogo grafico di formazione, competizioni, Scout, Trade Radar e asta.")

    run_report_button(
        "🧠 Genera Control Center",
        "Aggiorno tutti i motori principali...",
        "control_center_report_v3",
